...
- Menüleisten-App mit SF-System-Symbol als **Vektor-Template** (auflösungsfrei,
  passt sich Hell/Dunkel an). Statusanzeige direkt am Icon: gestoppt = Umschlag-
  Outline (`envelope`), läuft = gefüllt (`envelope.fill`), Zustellfehler =
  Umschlag mit Badge (`envelope.badge`). Fallback: gebündeltes Outline-PNG
  (macOS < 11). Kein Dock-Icon (`LSUIElement`)
